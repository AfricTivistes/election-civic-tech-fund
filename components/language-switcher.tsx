...
export default function LanguageSwitcher() {
  ...
  // Find current language with null check
  const currentLanguage =
    safeAvailableLanguages.find((lang) => lang && lang.code === language) || safeAvailableLanguages[0]

  // Function to generate URL with new language
  ...
  const handleLanguageChange = (langCode: string) => {
    try {
      if (langCode && (langCode === "fr" || langCode === "en") && typeof changeLanguage === "function") {
        // Wrap in Promise.resolve().catch to handle any potential promise rejections
        Promise.resolve(changeLanguage(langCode as "fr" | "en")).catch((error) => {
          console.error("Error in changeLanguage promise:", error)
        })
        setIsOpen(false)
      }
    } catch (error) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <span className="text-sm hidden sm:inline">{currentLanguage?.name || "Language"}</span>
