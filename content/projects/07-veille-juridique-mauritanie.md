---
id: "7"
slug: veille-juridique-mauritanie
projectName:
  fr: Veille et accompagnement juridique des jeunes aux processus électoraux
  en: Monitoring and Legal Support for Youth on Electoral Processes
organization: ADRES – Association pour le Développement et la Résilience Sociale
country:
  fr: Mauritanie
  en: Mauritania
countryCode: mr
countryFlag: 🇲🇷
category: major
budget: 15000
domain:
  fr: Accompagnement juridique et numérique
  en: Legal and Digital Support
description:
  fr: Le projet vise à outiller les jeunes de Nouakchott afin de renforcer leur
    accès à l'information juridique électorales.
  en: The project aims to equip young people in Nouakchott with improved access to
    electoral legal information.
impact:
  fr: Promouvoir une citoyenneté responsable et prévenir les tensions électorales
  en: Promote responsible citizenship and prevent electoral tensions
technologies:
  fr:
    - Legal Tech
    - Chatbot WhatsApp
    - Observatoire Citoyen
    - Documentaire
  en:
    - Legal Tech
    - WhatsApp Chatbot
    - Citizen Observatory
    - Documentary
video: https://youtube.com/shorts/qgRxHkI4Mz4
projectImage: /6.jpg
startDate: 2025-03-01
endDate: 2025-12-31
duration: 10
progress: 15
status: in_progress
featured: false
order: 7
tools:
  - url: https://wa.me/22248932192
    name: Chatbot juridique sur WhatsApp
    type: chatbot
    description:
      fr: >
        Un chatbot juridique adapté au contexte mauritanien permettant d'accéder
        à une information juridique fiable et accessible sur les droits et la
        participation électorale.
      en: >
        A legal chatbot tailored to the Mauritanian context, providing access to
        reliable and accessible legal information on rights and electoral
        participation.
---
## Outils déployés

Le projet déploie à Nouakchott, en particulier dans les quartiers marginalisés de Dar Naim et Toujounine, plusieurs outils simples, interactifs et durables pour renforcer l'accès des jeunes à l'information juridique électorale et stimuler leur participation citoyenne.

#### Observatoire citoyen numérique

Un comité de jeunes veilleurs produit des notes de veille et des recommandations sur les contentieux électoraux. L'observatoire documente les enjeux du scrutin et alimente le débat public avec des analyses accessibles, au service de la transparence.

#### Chatbot juridique WhatsApp

Un assistant conversationnel accessible via WhatsApp apporte des réponses vulgarisées sur les droits électoraux. En quelques messages, les citoyens obtiennent des explications simples sur les procédures, les recours et leurs garanties.

#### Campagne numérique ciblée

Une campagne sur les réseaux sociaux accompagne le chatbot pour maximiser la portée du projet auprès des jeunes des quartiers visés, en relayant des contenus pédagogiques et des messages de sensibilisation à la participation citoyenne.
